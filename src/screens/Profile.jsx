...
export default function Profile({ onNav }) {
  const { user, updateUser, logout, products } = usePantry()
  ...
  const stats = [
    { label: 'Productos', val: products.length },
    { label: 'Alertas', val: alertCount },
    { label: 'Frescos', val: freshCount },
  ]
  ...
  const infoRows = [
    ...
    { label: 'Miembro desde', value: `${MONTHS_ES[memberDate.getMonth()]} ${memberDate.getFullYear()}` },
  ...
  return (
    <div className="w-full h-full flex flex-col bg-gray-50 dark:bg-gray-950">
      <AppHeader title="Mi perfil" showBack onBack={() => onNav('settings')} />
      ...
      <div className="px-4 py-4 flex flex-col gap-4 flex-1 overflow-y-auto no-scrollbar">
        ...
        {editing ? (
          <form onSubmit={saveEdit} className="flex flex-col gap-3 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-lg p-4">
            ...
            <Field label="Correo electrónico" type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} />
            <Field label="Teléfono" placeholder="+57 300 000 0000" value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} />
            <div className="flex gap-2 mt-1">
              ...
              <div className="flex-1">
                <BtnPrimary type="submit" label="Guardar" />
              </div>
            </div>
          </form>
        ) : (
